...
def main():
    logger.info("=" * 70)
    logger.info("SIH26057 — STAGE 2: SSS IMAGE PREPROCESSING PIPELINE")
    logger.info("=" * 70)
    ...
    if os.path.exists(cfg_path):
        with open(cfg_path, "r") as f:
            ...
            logger.info(f"[1/4] Loaded preprocessing configuration from {cfg_path}")
    ...
    logger.info(f"  Processed: {batch_summary['successfully_processed']} images in {batch_summary['total_runtime_seconds']}s")
    logger.info(f"  Average latency: {batch_summary['avg_latency_ms_per_image']} ms/image")
    logger.info(f"  Average Equivalent Number of Looks (ENL) gain: {batch_summary['avg_enl_improvement']}x (speckle reduction)")
    logger.info(f"  Average contrast gain: {batch_summary['avg_contrast_gain']}x")
    ...
    if os.path.exists(monrovia_path):
        ...
        logger.info(f"  Generated {len(tiles)} overlapping 640x640 inference tiles from Monrovia waterfall")
        logger.info(f"  Monrovia preprocessed saved to: {m_out_path}")

    # 5. Summary
    logger.info("\n[4/4] Output Visualizations & Reports:")
    logger.info(f"  Side-by-side comparison images: {os.path.join(output_dir, 'comparisons')}")
    logger.info(f"  Extracted acoustic shadow masks: {os.path.join(output_dir, 'shadows')}")
    logger.info(f"  Summary report: {os.path.join(output_dir, 'batch_preprocessing_summary.json')}")

    logger.info("\n" + "=" * 70)
    logger.info("STAGE 2 PREPROCESSING COMPLETED SUCCESSFULLY!")
    logger.info("=" * 70)
