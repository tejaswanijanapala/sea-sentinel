...
logger.info('Meta:', meta.latitude, meta.longitude, meta.sonar_range, meta.altitude, meta.heading)
for d in res['detections']:
    bbox = d.get('bbox')
    logger.info('Raw d bbox:', bbox, type(bbox))
    g = georef.georeference_detection(bbox, (640, 640), meta)
    logger.info(f"Detection {d.get('object_id')}: lat={g.latitude}, lon={g.longitude}, ground_range={g.ground_range_m}, bearing={g.bearing_deg}")
