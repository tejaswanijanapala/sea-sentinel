...
def test_api_pipeline():
    ...
    logger.info("Status:", res.get("status"))
    logger.info("Analysis ID:", res.get("analysis_id"))
    logger.info("Detections count:", len(res.get("detections", [])))
    logger.info("Raw Image URL:", res.get("raw_image_url"))
    logger.info("Enhanced Image URL:", res.get("enhanced_image_url"))
    logger.info("Center WGS84:", res.get("center_wgs84"))
    ...
    if res.get("raw_image_url"):
        ...
        with urllib.request.urlopen(raw_full) as r_img:
            ...
            logger.info("Raw Image stream HTTP 200 OK")
    ...
    if res.get("enhanced_image_url"):
        ...
        with urllib.request.urlopen(enh_full) as e_img:
            ...
            logger.info("Enhanced Image stream HTTP 200 OK")
    ...
    with urllib.request.urlopen(gis_url) as g_resp:
        ...
        logger.info("GIS Database targets count:", len(g_data.get("targets", [])))
